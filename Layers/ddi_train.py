...
df = 20
quantile = 0.98
chi2_value = chi2.ppf(quantile, df)
...
def evaluate(model, loader, set_len):
    ...
    for batch in loader:
        graph_batch_1, graph_batch_2, ddi_type, y_true,cw,yxj = batch

        y_pred,mmr = model.forward_func(graph_batch_1, graph_batch_2, ddi_type,[1,0],[0,1],0)

        # 计算sigmoid后的概率，用于二进制分类
    ...
    with open(filename, 'a') as file:
        for epoch in range(args.num_epoch):
            ...
            for i, batch in enumerate(train_loader):
                graph_batch_1, graph_batch_2, ddi_type, y_true,cnm,cnm1 = batch
                y_true = torch.Tensor(y_true).to(args.device)
                ccccccccccc=graph_batch_1.x
                y_pred,y_pred1 = model.forward_func(graph_batch_1, graph_batch_2, ddi_type,cnm,cnm1,1)
                print(ccccccccccc==graph_batch_1.x)
                predicted_labels = (y_pred1 <= chi2_value).float()
                loss1 = torch.mean((predicted_labels != y_true).float())
                #print('loss1',loss1)
                loss = 0.3*criterion(y_pred, y_true)+0.7*loss1
